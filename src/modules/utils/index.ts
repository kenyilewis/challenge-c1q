...
class CustomError extends Error {
  ...
  constructor(error: IError) {
    super(error.message);
    this.error = error.error || 'internal_server_error';
    this.status = error.status || 500;
  }
}

export const handlerError = (error: IError): never => {
...
const luhnAlgorithm = (cardNumber: number): boolean => {
  ...
  const isValid = digits.reduce(
    (accumulator: number, digit: number, index: number) => {
      if (index % 2 === 1) {
        digit *= 2;
...
export const validateAndParseCard = async (data: ICard): Promise<ICard> => {
  ...
  if (
    !luhnAlgorithm(card_number) ||
    !validator.isLength(card_number.toString(), { min: 13, max: 16 })
  ) {
    handlerError({
      status: 400,
      error: 'bad_request',
  ...
  if (
    !validator.isLength(cvv.toString(), { min: 3, max: 4 }) ||
    !validator.isNumeric(cvv.toString())
  ) {
    handlerError({
      status: 400,
      error: 'bad_request',
  ...
  if (
    (!validator.isAfter(expiration_month, '0') &&
      !validator.isBefore(expiration_month, '13')) ||
  ...
  if (
    !validator.isAfter(expiration_year, yearNow) ||
    !validator.isBefore(expiration_year, yearBefore) ||
